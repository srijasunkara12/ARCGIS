...
for yr = yrs_all'
    ...
    for t = LAG+1 : n_yr
        ...
        % 49 rainfall + inflow + basin rain + rain3 + rain5 + baseflow
        row = zeros(1,(49+5)*LAG);
        ii = 1;
        ...
        for lag = 1:LAG
            ...
            % basin rainfall
            row(ii) = T_yr.BasinRain(t-lag);
            ii = ii + 1;

            % storm accumulation
            row(ii) = T_yr.Rain3(t-lag);
            ii = ii + 1;

            row(ii) = T_yr.Rain5(t-lag);
            ii = ii + 1;
...
POP_SIZE  = 1000;
N_GENS    = 300;
MAX_DEPTH = 10;     % deeper trees = more expressive models
CX_PROB   = 0.85;  % high crossover drives exploration
MUT_PROB  = 0.10;  % moderate mutation maintains diversity
ELITISM   = 20;    % preserve top 10 trees each generation
TOURN_K   = 9;     % stronger selection pressure
