...
%      - R2 penalty kicks in strongly below 0.8598  (test target)
...
%      - Halt if train R2 > 0.9002 AND test R2 > 0.8598
%
...
station_cols = arrayfun(@(i)sprintf('Station_%d',i),1:37,'UniformOutput',false);
rain_matrix  = zeros(height(T_all),37);
for s = 1:37
...
%% ========================================================================
LAG       = 5;
N_STATIONS = 37;  % 37 rain gauge stations  (input shape: 37 x 122)
FEATS_LAG = 50;   % 37 stations + 13 engineered features per lag
...
n_feat = size(X_all,2);
fprintf('  Samples: %d  |  Features: %d  (37 stations x 122 days -> 37x122 input)\n', size(X_all,1), n_feat);
...
N_GENS           = 600;    % more generations — affordable with 37 stations
...
EARLY_STOP_TR    = 0.9002;
EARLY_STOP_TE    = 0.8598;
...
arch_box(ax1,0.3,12.5,9.4,1.2,...
    sprintf('INPUT: 37 Stations x 5 Lags + AMI + Trend + Anomaly + WetDays + ... = %d Features  |  Monsoon Only (Jun-Oct)  |  37x122 data',n_feat),...
    [0.08 0.39 0.75]);
for li = 1:5
...
station_imp=zeros(37,1); lag_imp=zeros(LAG,1);
for lag=1:LAG
    for s=1:37
