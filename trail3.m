...
% scale to [0,1]
X_min = min(X_all,[],1);  X_max = max(X_all,[],1);
X_sc  = (X_all - X_min) ./ max(X_max-X_min, 1e-12);
...
POP_SIZE  = 500;   % larger population = more diversity
N_GENS    = 150;   % more generations = better convergence
MAX_DEPTH = 7;     % deeper trees = more expressive models
...
function f=gp_fitness(nd,X,y)
    try
        ...
        penalty = max(0, gp_tree_size(nd)-60) * 0.0003;
        f = (1 - nse) + 0.1*rmse + penalty;
        if f < 0, f = 0; end
    catch, f=1e9; end
end
