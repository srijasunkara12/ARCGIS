...
%  TARGET: R2_train = 0.8950 , R2_test >= 0.8575
%
...
%      - R2 penalty kicks in strongly below 0.90  (from v2)
...
%      - Halt if train R2 > 0.93 AND test R2 > 0.91
%
...
arch_box(ax1,0.3,2.7,9.4,1.0,...
    'Fitness = (1-NSE)^2 + 0.01*RMSE + R2_penalty(<0.90) + PeakBonus(x3) + AdaptiveParsimony',...
    [0.55 0.20 0.05]);
...
text(ax22,gens_v(max(1,round(N_GENS*0.05))),0.915,'R^2=0.90 target',...
...
function f=gp_fitness(nd,X,y,peak_mask,size_limit,parsimony_coef)
    ...
    %          + r2_penalty (heavy kick when R2 < 0.90)
    ...
    try
        ...
        % Strong penalty when R2 dips below 0.90
        r2_penalty = 0;
        if nse < 0.90
            r2_penalty = 5*(0.90-nse)^2;
        end
...
function f=local_const_obj(tree,const_paths,c,X,y,peak_mask)
    ...
    rmse=sqrt(mean((y-pred).^2));
    pk_err=mean((y(peak_mask)-pred(peak_mask)).^2);
    r2_pen=0; if nse<0.90, r2_pen=5*(0.90-nse)^2; end
