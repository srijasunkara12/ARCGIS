...
%  TARGET: R2_train = 0.907 , R2_test = 0.87
%
...
%      - R2 penalty kicks in strongly below 0.9435  (test target)
...
N_GENS           = 1300;    % more generations — affordable with 37 stations
MAX_DEPTH        = 14;     % conservative cap prevents bloat / overfitting
CX_PROB          = 0.85;   % slightly higher crossover for exploitation
MUT_PROB_START   = 0.15;   % decays to 0.04 (tighter end)
ELITISM          = 40;     % more elites preserved each generation
TOURN_K_LO       = 5;
TOURN_K_HI       = 15;     % higher late-gen pressure drives convergence
...
text(ax22,gens_v(max(1,round(N_GENS*0.05))),0.870,'R^2=0.9200 target (test)',...
...
function f=gp_fitness(nd,X,y,peak_mask,size_limit,parsimony_coef)
    ...
    try
        ...
        r2_penalty = 0;
        if nse < 0.88
            r2_penalty = 20*(0.85-nse)^2;
        elseif nse < 0.75
            r2_penalty = 50*(0.75-nse)^2;
        end
