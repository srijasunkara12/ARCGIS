%% ========================================================================
%  GENETIC PROGRAMMING FOR RAINFALL-INFLOW PREDICTION  *** v3 MERGED ***
%  TARGET: R2_train = 0.9002 , R2_test = 0.8598
%
%  Godavari Basin | 49 Rain Gauge Stations | 5-Day Lag
%  MONSOON ONLY (June-October) | 80% Train / 20% Test
%
%  IMPROVEMENTS OVER v2 (document 2):
%  [A] FEATURE ENGINEERING
%      - Antecedent Moisture Index AMI (exp decay k=0.85)  <- from v2-improved
%      - Inflow trend delta(Q)  <- from v2-improved
%      - Rainfall anomaly vs monsoon climatology  <- from v2-improved
%      - Rain7, RainMax, RainStd retained from v2
%      - WetDays count (stations > 1 mm)  <- new
%      Total: 62 features x 5 lags = 310 features
%
%  [B] FITNESS FUNCTION  (KEY IMPROVEMENT)
%      - Base: (1-NSE)^2 + 0.01*RMSE
%      - R2 penalty kicks in strongly below 0.8575  (from v2)
%      - Peak-flow double-weight: top-10% events weighted 3x  <- new
%      - Adaptive parsimony: grows from 0.0001 to 0.0005 over gens  <- new
%      - Best individual tracked by TEST R2, not train fitness  (from v2)
%
%  [C] POPULATION & OPERATORS
%      - Pop=1500, Gens=400, Elitism=30  (from v2)
%      - Operator set: +,-,*,/,sqrt,sq,log,cube,tanh,abs (10 ops)  <- expanded
%      - Hoist mutation (size reduction)  <- from v2-improved
%      - Constant perturbation mutation  <- from v2-improved
%      - Adaptive tournament k=5->9 over generations  <- from v2-improved
%      - Adaptive mutation rate: 0.15 -> 0.05  <- from v2-improved
%
%  [D] DOMAIN SEEDS (7 hand-crafted trees)
%      - Persistence, Linear, Sqrt-Rain, Baseflow, AMI,
%        Quadratic-peak, Linear-combo  <- all merged
%
%  [E] STAGNATION RESTART  (from v2)
%      - After 50 no-improve gens: inject 30% fresh trees
%
%  [F] EARLY STOP  (from v2)
%      - Halt if train R2 > 0.8950 AND test R2 > 0.8575
%
%  [G] POST-GP CONSTANT OPTIMISATION  (from v2-improved)
%      - Nelder-Mead on all constants of best tree
%
%  [H] MULTI-RESTART  (3 independent runs, best kept)  <- from v2-improved
%
%  [I] FIGURES (6 total, identical styling, + R2 panel in Fig2)
% ========================================================================
clc; clear; close all;
...
%%
%%  Features per lag (FEATS_LAG = 62):
%%   1-49 : Station rain       50: prev_inflow    51: BasinRain
%%   52: Rain3   53: Rain5     54: Rain7          55: RainMax
%%   56: RainStd 57: WetDays   58: AMI            59: RainAnom
%%   60: BaseFlow(5-day mean)  61: InflowTrend    62: (spare=0)
%% ========================================================================
LAG       = 5;
FEATS_LAG = 62;
...
CX_PROB          = 0.80;
MUT_PROB_START   = 0.15;   % decays to 0.05
ELITISM          = 30;
TOURN_K_LO       = 5;
TOURN_K_HI       = 9;
STAGNATION_LIMIT = 50;
PARSIMONY_START  = 0.0001;
PARSIMONY_END    = 0.0005;
SIZE_LIMIT       = 50;
EARLY_STOP_TR    = 0.8950;
EARLY_STOP_TE    = 0.8575;

OPS   = {'+','-','*','/','sqrt','sq','log','cube','tanh','abs'};
ARITY = [ 2,  2,  2,  2,    1,   1,    1,    1,     1,   1];

% Convenient feature index helpers (lag-1)
F_INF1  = FEATS_LAG*0 + 50;   % lag-1 inflow
F_BAS1  = FEATS_LAG*0 + 51;   % lag-1 BasinRain
F_R51   = FEATS_LAG*0 + 53;   % lag-1 Rain5
F_R71   = FEATS_LAG*0 + 54;   % lag-1 Rain7
F_AMI1  = FEATS_LAG*0 + 58;   % lag-1 AMI
F_BF1   = FEATS_LAG*0 + 60;   % lag-1 baseflow
F_MAX1  = FEATS_LAG*0 + 55;   % lag-1 RainMax
...
for restart = 1:N_RESTARTS
    ...
    for gen = 1:N_GENS
        ...
        mut_prob = MUT_PROB_START - (MUT_PROB_START-0.05)*frac;
...
fig1 = figure('Color',[0.05 0.07 0.09],'Position',[60 60 1440 730],...
...
arch_box(ax1,0.3,2.7,9.4,1.0,...
    'Fitness = (1-NSE)^2 + 0.01*RMSE + R2_penalty(<0.8575) + PeakBonus(x3) + AdaptiveParsimony',...
    [0.55 0.20 0.05]);
...
yline(ax22,0.8575,'w--','LineWidth',1.5);
text(ax22,gens_v(max(1,round(N_GENS*0.05))),0.868,'R^2=0.8575 target (test)',...
...
for tt = 1:2
    ...
    figX = figure('Color','k','Position',[40 40 1800 500],...
...
fig5=figure('Color','k','Position',[60 60 1500 950],'Name','Error Analysis','NumberTitle','off');
...
yline(ax57,0.8575,'w--','LineWidth',1.5);
...
sgtitle(fig5,'Error Analysis — GP Model v3 (Monsoon)','Color','w','FontSize',13,'FontWeight','bold');
...
function f=gp_fitness(nd,X,y,peak_mask,size_limit,parsimony_coef)
    ...
    %          + r2_penalty (heavy kick when R2 < 0.8575)
    ...
    try
        ...
        % Strong penalty when R2 dips below target (0.8575)
        r2_penalty = 0;
        if nse < 0.8575
            r2_penalty = 5*(0.8575-nse)^2;
        end
...
function f=local_const_obj(tree,const_paths,c,X,y,peak_mask)
    ...
    rmse=sqrt(mean((y-pred).^2));
    pk_err=mean((y(peak_mask)-pred(peak_mask)).^2);
    r2_pen=0; if nse<0.8575, r2_pen=5*(0.8575-nse)^2; end
