...
%% ---- Generate & display the best-fit symbolic equation ----------------
eq_str = gp_to_string(best_tree);
fprintf('\n=== Best-fit Symbolic Equation ===\n%s\n', eq_str);

% Save equation to text file
fid = fopen(fullfile(output_dir,'GP_Equation_v3.txt'), 'w');
fprintf(fid, 'GP v3 Best-fit Equation\n');
fprintf(fid, '========================\n');
fprintf(fid, 'Train R2 = %.4f  |  Test R2 = %.4f\n', global_best_r2te, new_r2te);
fprintf(fid, 'Tree size = %d nodes, depth = %d\n\n', ...
    gp_tree_size(best_tree), gp_tree_depth(best_tree));
fprintf(fid, 'Equation:\n%s\n\n', eq_str);
fprintf(fid, 'Feature index legend (lag-1 block):\n');
fprintf(fid, '  X1-X49  = Station rainfall (lag 1)\n');
fprintf(fid, '  X50     = prev Grand_Total_Inflow\n');
fprintf(fid, '  X51     = BasinRain\n');
fprintf(fid, '  X52     = Rain3\n');
fprintf(fid, '  X53     = Rain5\n');
fprintf(fid, '  X54     = Rain7\n');
fprintf(fid, '  X55     = RainMax\n');
fprintf(fid, '  X56     = RainStd\n');
fprintf(fid, '  X57     = WetDays\n');
fprintf(fid, '  X58     = AMI\n');
fprintf(fid, '  X59     = RainAnom\n');
fprintf(fid, '  X60     = BaseFlow (5-day mean)\n');
fprintf(fid, '  X61     = InflowTrend (delta Q)\n');
fprintf(fid, '  (Each subsequent lag block adds 62 to each index)\n');
fclose(fid);
fprintf('  Equation saved: GP_Equation_v3.txt\n');
...
set(ax2,'Color','w','XLim',[0 10],'YLim',[0 10],...
    'XTick',[],'YTick',[],'Box','off');
title(ax2,'Symbolic Expression Tree (Best Found)','Color','k','FontSize',11,'FontWeight','bold');
...
for ni = 1:length(fn2)
    ...
    rectangle(ax2,'Position',[pos(1)-0.60 pos(2)-0.44 1.20 0.88],...
              'Curvature',0.5,'FaceColor',fc,'EdgeColor','k','LineWidth',1.4);
...
for ci=1:3
    ...
    rectangle(ax2,'Position',[lx+(ci-1)*1.5 ly 1.0 0.55],'Curvature',0.4,...
              'FaceColor',clrs{ci},'EdgeColor','k');
    text(ax2,lx+(ci-1)*1.5+0.5,ly+0.27,lbls{ci},...
         'Color','w','HorizontalAlignment','center','FontSize',7.5);
end

% ---- Best-fit equation annotation on Fig 1 ----------------------------
eq_display = eq_str;
if length(eq_str) > 90
    eq_display = [eq_str(1:87), '...'];
end
text(ax2, 5, 0.60, sprintf('f(X) = %s', eq_display), ...
     'Color','k','HorizontalAlignment','center',...
     'FontSize',7.5,'FontName','Courier',...
     'Interpreter','none',...
     'BackgroundColor',[0.94 0.98 0.94],...
     'EdgeColor',[0.50 0.75 0.50],'LineWidth',1.0);
...
text(ax2,5,0.10,...
    ...
    'BackgroundColor',[0.95 0.95 0.95],'EdgeColor',[0.70 0.70 0.70]);
...
%% ====================================================================
%%  FIGURE 2 — CONVERGENCE  (WHITE BACKGROUND)
%% ====================================================================
...
ax21 = subplot(1,4,1);
plot(ax21,gens_v,h_bfit(1:N_GENS),'Color',[0.05 0.55 0.18],'LineWidth',2,'DisplayName','Train Fit'); hold(ax21,'on');
plot(ax21,gens_v,h_trmse(1:N_GENS),'Color',[0.85 0.33 0.00],'LineWidth',2,'DisplayName','Test RMSE');
lg=legend(ax21,'Location','northeast','FontSize',8,'TextColor','k'); lg.Color='w'; lg.EdgeColor='k';
...
ax22 = subplot(1,4,2);
plot(ax22,gens_v,h_r2tr(1:N_GENS),'Color',[0.00 0.45 0.70],'LineWidth',2,'DisplayName','Train R^2'); hold(ax22,'on');
plot(ax22,gens_v,h_r2te(1:N_GENS),'Color',[0.80 0.10 0.10],'LineWidth',2,'DisplayName','Test R^2');
yline(ax22,0.8598,'k--','LineWidth',1.5);
text(ax22,gens_v(max(1,round(N_GENS*0.05))),0.870,'R^2=0.8598 target',...
     'Color','k','FontSize',8,'Interpreter','none');
lg2=legend(ax22,'Location','southeast','FontSize',8,'TextColor','k'); lg2.Color='w'; lg2.EdgeColor='k';
...
ax23 = subplot(1,4,3);
plot(ax23,gens_v,h_afit(1:N_GENS),'Color',[0.50 0.18 0.75],'LineWidth',2);
...
ax24 = subplot(1,4,4);
plot(ax24,gens_v,h_asiz(1:N_GENS),'Color',[0.00 0.55 0.65],'LineWidth',2);
...
%% ====================================================================
C_obs=[0.18 0.42 0.70]; C_pred=[0.85 0.33 0.00];
...
for tt = 1:2
    ...
    for yi = 1:length(yrs_v)
        ...
        if mod(yi,2)==0
            patch(axA,[xs xe xe xs],[0 0 1 1]*pk_o*1.12,...
                  [0.92 0.92 0.92],'FaceAlpha',1,'EdgeColor','none','HandleVisibility','off');
        end
    ...
    scatter(axA,xv(pk_pi),pk_p,100,[0.6 0.2 0.8],'filled','HandleVisibility','off');
    text(axA,xv(pk_oi),pk_o*1.03,sprintf('Obs Peak: %.0f',pk_o),...
         'Color','k','FontSize',8,'FontWeight','bold','HorizontalAlignment','center');
    text(axA,xv(pk_pi),pk_p*1.03,sprintf('Pred Peak: %.0f',pk_p),...
         'Color',[0.5 0.0 0.7],'FontSize',8,'FontWeight','bold','HorizontalAlignment','center');
    ...
    set(axA,'Color','w','XColor','k','YColor','k','GridColor',[0.80 0.80 0.80],...
            'GridAlpha',1,'FontSize',8,'LineWidth',0.8,'Box','on','TickDir','out');
    grid(axA,'on');
    lg=legend(axA,'Location','northeast','FontSize',10,'TextColor','k');
...
ax4a=subplot(2,2,1);
scatter(ax4a,train_obs,train_pred,28,[0.10 0.60 0.25],'filled','MarkerFaceAlpha',0.6); hold(ax4a,'on');
...
ax4b=subplot(2,2,2);
scatter(ax4b,test_obs,test_pred,28,[0.85 0.33 0.00],'filled','MarkerFaceAlpha',0.6); hold(ax4b,'on');
...
scatter(ax4c,train_obs,train_pred,22,[0.10 0.60 0.25],'filled','MarkerFaceAlpha',0.5,'DisplayName','Train'); hold(ax4c,'on');
scatter(ax4c,test_obs, test_pred, 22,[0.85 0.33 0.00],'filled','MarkerFaceAlpha',0.5,'DisplayName','Test');
...
lg4=legend(ax4c,'Location','northwest','FontSize',9,'TextColor','k');
lg4.Color='w'; lg4.EdgeColor='k';
bax(ax4c);
...
ax51=subplot(3,3,1); bar(ax51,tr_err,'FaceColor',[0.75 0.15 0.15],'EdgeColor','none','FaceAlpha',0.85);
yline(ax51,0,'k','LineWidth',1); dbax(ax51,'Sample Index','Error (cusecs)','Train Residuals');

ax52=subplot(3,3,2); bar(ax52,te_err,'FaceColor',[0.10 0.40 0.75],'EdgeColor','none','FaceAlpha',0.85);
yline(ax52,0,'k','LineWidth',1); dbax(ax52,'Sample Index','Error (cusecs)','Test Residuals');

ax53=subplot(3,3,3);
histogram(ax53,tr_err,25,'FaceColor',[0.10 0.60 0.25],'FaceAlpha',0.8,'EdgeColor','none','DisplayName','Train'); hold(ax53,'on');
histogram(ax53,te_err,25,'FaceColor',[0.85 0.33 0.00],'FaceAlpha',0.8,'EdgeColor','none','DisplayName','Test');
xline(ax53,0,'k--','LineWidth',1.5);
lg53=legend(ax53,'FontSize',8,'TextColor','k'); lg53.Color='w'; lg53.EdgeColor='k';
...
scatter(ax54,x_tr(pk_pi_tr),train_pred(pk_pi_tr),100,[0.5 0.0 0.7],'filled','HandleVisibility','off');
lg54=legend(ax54,'Location','northwest','FontSize',7,'TextColor','k'); lg54.Color='w'; lg54.EdgeColor='k';
...
scatter(ax55,x_te(pk_pi_te),test_pred(pk_pi_te),100,[0.5 0.0 0.7],'filled','HandleVisibility','off');
lg55=legend(ax55,'Location','northwest','FontSize',7,'TextColor','k'); lg55.Color='w'; lg55.EdgeColor='k';
dbax(ax55,'Sample','Inflow (cusecs)','Test — Peak Discharge');

ax56=subplot(3,3,6); mk={'RMSE','MAE','MAPE'}; xb=1:3; w=0.35;
bar(ax56,xb-w/2,[m_tr.RMSE m_tr.MAE m_tr.MAPE],w,'FaceColor',[0.10 0.60 0.25],'EdgeColor','none','DisplayName','Train'); hold(ax56,'on');
bar(ax56,xb+w/2,[m_te.RMSE m_te.MAE m_te.MAPE],w,'FaceColor',[0.85 0.33 0.00],'EdgeColor','none','DisplayName','Test');
set(ax56,'XTick',xb,'XTickLabel',mk); ax56.XAxis.TickLabelColor='k';
lg56=legend(ax56,'FontSize',8,'TextColor','k'); lg56.Color='w'; lg56.EdgeColor='k';
dbax(ax56,'','Value','Error Metrics');

ax57=subplot(3,3,7); mk2={'R^2','NSE','Corr'};
bar(ax57,xb-w/2,[m_tr.R2 m_tr.NSE m_tr.Corr],w,'FaceColor',[0.10 0.60 0.25],'EdgeColor','none','DisplayName','Train'); hold(ax57,'on');
bar(ax57,xb+w/2,[m_te.R2 m_te.NSE m_te.Corr],w,'FaceColor',[0.85 0.33 0.00],'EdgeColor','none','DisplayName','Test');
set(ax57,'XTick',xb,'XTickLabel',mk2); ylim(ax57,[-0.2 1.2]); ax57.XAxis.TickLabelColor='k';
yline(ax57,0.8598,'k--','LineWidth',1.5);
lg57=legend(ax57,'FontSize',8,'TextColor','k'); lg57.Color='w'; lg57.EdgeColor='k';
dbax(ax57,'','Value','Goodness-of-Fit');

ax58=subplot(3,3,8);
plot(ax58,x_tr,cumsum(abs(tr_err)),'Color',[0.10 0.65 0.30],'LineWidth',1.8,'DisplayName','Train'); hold(ax58,'on');
plot(ax58,x_te,cumsum(abs(te_err)),'Color',[0.85 0.33 0.00],'LineWidth',1.8,'DisplayName','Test');
lg58=legend(ax58,'FontSize',8,'TextColor','k'); lg58.Color='w'; lg58.EdgeColor='k';
...
text(ax59,0.45,ys(1),'Train','FontWeight','bold','FontSize',10,'Units','normalized','Color',[0.10 0.55 0.20]);
text(ax59,0.73,ys(1),'Test', 'FontWeight','bold','FontSize',10,'Units','normalized','Color',[0.75 0.28 0.00]);
for ri = 1:length(rows)
    bg=[0.97 0.97 0.97]; if mod(ri,2)==0, bg=[0.92 0.92 0.92]; end
    text(ax59,0.05,ys(ri+1),rows{ri},                 'FontSize',9,'Units','normalized','BackgroundColor',bg,'Color','k');
    text(ax59,0.45,ys(ri+1),sprintf('%.3f',tr_v(ri)), 'FontSize',9,'Units','normalized','BackgroundColor',bg,'Color',[0.10 0.55 0.20]);
    text(ax59,0.73,ys(ri+1),sprintf('%.3f',te_v(ri)), 'FontSize',9,'Units','normalized','BackgroundColor',bg,'Color',[0.75 0.28 0.00]);
end
...
BG=[1 1 1]; FG=[0 0 0]; GRD=[0.80 0.80 0.80];
fig6=figure('Color',BG,'Position',[60 60 1300 600],...
            'Name','Feature Importance','NumberTitle','off');

ax6L=subplot(1,2,1);
barh(ax6L,simp_s(top_n:-1:1),'FaceColor',[0.12 0.42 0.72],'EdgeColor','none','FaceAlpha',1.0);
yticks(ax6L,1:top_n);
...
lag_clr=[0.80 0.35 0.05;0.75 0.10 0.10;0.05 0.55 0.75;0.10 0.60 0.20;0.50 0.10 0.65];
...
%% ====================================================================
%%  FIGURE 7 — BEST-FIT EQUATION DISPLAY  (WHITE BACKGROUND)
%% ====================================================================
fig7 = figure('Color','w','Position',[60 60 1000 400],...
              'Name','Best-Fit Equation','NumberTitle','off');
ax7  = axes(fig7,'Position',[0.02 0.12 0.96 0.72]);
set(ax7,'Visible','off','Color','w');

title(ax7, sprintf('GP v3 — Symbolic Equation  |  Train R^2=%.4f  |  Test R^2=%.4f  |  Nodes=%d  Depth=%d',...
      m_tr.R2, m_te.R2, gp_tree_size(best_tree), gp_tree_depth(best_tree)), ...
      'FontSize',11,'FontWeight','bold','Color','k','Interpreter','none');

raw_eq  = eq_str;
wrap_w  = 75;
lines_eq = {};
while length(raw_eq) > wrap_w
    cut = find(raw_eq(1:wrap_w)==' ', 1, 'last');
    if isempty(cut), cut = wrap_w; end
    lines_eq{end+1} = strtrim(raw_eq(1:cut)); %#ok<AGROW>
    raw_eq = strtrim(raw_eq(cut+1:end));
end
lines_eq{end+1} = raw_eq;
eq_wrapped = strjoin(lines_eq, newline);

text(ax7, 0.5, 0.52, eq_wrapped, ...
     'HorizontalAlignment','center','VerticalAlignment','middle', ...
     'FontSize',9,'FontName','Courier','Interpreter','none', ...
     'Color','k','Units','normalized', ...
     'BackgroundColor',[0.96 0.98 0.96],'EdgeColor',[0.55 0.75 0.55],...
     'Margin',8,'LineWidth',1.0);

legend_str = sprintf(['Feature index guide (lag-1 block):  ' ...
    'X38=prev Inflow   X39=BasinRain   X41=Rain5   X42=Rain7   ' ...
    'X43=RainMax   X46=AMI   X48=BaseFlow   X49=InflowTrend']);
text(ax7, 0.5, 0.02, legend_str, ...
     'HorizontalAlignment','center','VerticalAlignment','bottom', ...
     'FontSize',8,'FontName','Courier','Interpreter','none', ...
     'Color',[0.35 0.35 0.35],'Units','normalized');

sgtitle(fig7,'GP v3 — Best-fit Symbolic Equation','Color','k','FontSize',13,'FontWeight','bold');
saveas(fig7, fullfile(output_dir,'Fig7_BestFit_Equation.png'));
fprintf('  Saved Fig7_BestFit_Equation.png\n');

%% ====================================================================
%%  FIGURE 8 — SENSITIVITY ANALYSIS  (FIXED)
%% ====================================================================
FIG_BG = [1 1 1];
FG     = [0 0 0];
GRD    = [0.80 0.80 0.80];
...
PERTURB   = 0.10;
base_pred = gp_eval(best_tree, X_test);

sens_pos  = zeros(n_feat, 1);
sens_neg  = zeros(n_feat, 1);
...
%% --- Map feature index to human-readable label -----------------------
...
%% --- PANEL A: Top-30 OAT bar chart  (FIX APPLIED HERE) --------------
fig8 = figure('Color', FIG_BG, 'Position', [60 60 1600 900], ...
              'Name', 'Sensitivity Analysis', 'NumberTitle', 'off');

ax8a = subplot(2, 2, [1 3]);
hold(ax8a, 'on');

% ypos must be strictly increasing for MATLAB's YTick
% Reverse the data arrays instead so rank-1 appears at the top visually
ypos = (1 : TOP_N_SENS)';          % ascending — required by MATLAB

top_pos_r  = top_pos(end:-1:1);    % rank-1 (highest) plotted at top (highest y)
top_neg_r  = top_neg(end:-1:1);
top_avg_r  = top_avg(end:-1:1);
top_lbls_r = top_lbls(end:-1:1);   % matching labels, reversed

barh(ax8a, ypos - 0.18, top_pos_r, 0.30, ...
     'FaceColor', [0.88 0.25 0.15], 'EdgeColor', 'none', ...
     'FaceAlpha', 0.90, 'DisplayName', '+10%');
barh(ax8a, ypos + 0.18, top_neg_r, 0.30, ...
     'FaceColor', [0.09 0.55 0.82], 'EdgeColor', 'none', ...
     'FaceAlpha', 0.90, 'DisplayName', '-10%');
barh(ax8a, ypos, top_avg_r, 0.06, ...
     'FaceColor', FG, 'EdgeColor', 'none', 'HandleVisibility', 'off');

set(ax8a, 'YTick', ypos, 'YTickLabel', top_lbls_r, ...
          'FontSize', 8);

xlabel(ax8a, 'Mean |Delta Output| (scaled)', 'Color', FG, 'FontSize', 10, 'FontWeight', 'bold');
...
ylabel(ax8b, 'Mean |Delta Output| (avg over features)', ...
       'Color', FG, 'FontSize', 9);
title(ax8b, 'Sensitivity by Lag Depth', ...
      'Color', FG, 'FontSize', 11, 'FontWeight', 'bold');
bax_w(ax8b);

%% --- PANEL C: Category-level bar (scalar features only) -------------
ax8c = subplot(2, 2, 4);
...
set(ax8c, 'XTick', 1:n_cats, ...
          ...
          'FontSize', 8);
ylabel(ax8c, 'Mean |Delta Output| (avg over lags)', ...
       'Color', FG, 'FontSize', 9);
...
%% --- Symbolic equation string converter --------------------------------
function str=gp_to_string(nd)
    switch nd.type
        case 'var'
        ...
        case 'func'
            switch nd.op
                case '+'
                    str = sprintf('(%s + %s)', ...
                        gp_to_string(nd.ch{1}), gp_to_string(nd.ch{2}));
                case '-'
                    str = sprintf('(%s - %s)', ...
                        gp_to_string(nd.ch{1}), gp_to_string(nd.ch{2}));
                case '*'
                ...
                case '/'
                    str = sprintf('(%s / %s)', ...
                        gp_to_string(nd.ch{1}), gp_to_string(nd.ch{2}));
                case 'sqrt'
                    str = sprintf('sqrt(|%s|)', gp_to_string(nd.ch{1}));
                case 'sq'
                    str = sprintf('(%s)^2', gp_to_string(nd.ch{1}));
                case 'cube'
                    str = sprintf('(%s)^3', gp_to_string(nd.ch{1}));
                case 'log'
                    str = sprintf('log(|%s|)', gp_to_string(nd.ch{1}));
                case 'tanh'
                    str = sprintf('tanh(%s)', gp_to_string(nd.ch{1}));
                case 'abs'
                    str = sprintf('|%s|', gp_to_string(nd.ch{1}));
                otherwise
                    str = sprintf('%s(%s)', nd.op, gp_to_string(nd.ch{1}));
            end
        otherwise
            str = '?';
    end
...
function arch_box(ax,x,y,w,h,txt,fc)
    rectangle(ax,'Position',[x y w h],'Curvature',0.08,...
              'FaceColor',fc,'EdgeColor','k','LineWidth',1.4);
...
function bax(axh)
    set(axh,'Color','w','XColor','k','YColor','k',...
            'GridColor',[0.80 0.80 0.80],'GridAlpha',1,...
            'FontSize',9,'LineWidth',0.8,'Box','on','TickDir','out');
    grid(axh,'on');
end

function bax_w(axh)
    set(axh,'Color',[1 1 1],'XColor',[0 0 0],'YColor',[0 0 0],...
            'GridColor',[0.80 0.80 0.80],'GridAlpha',1,...
            'FontSize',9,'LineWidth',0.8,'Box','on','TickDir','out');
    grid(axh,'on');
end
